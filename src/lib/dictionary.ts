export type Language = "UZ" | "RU" | "EN";

export const dictionary = {
  UZ: {
    navbar: {
      features: "Imkoniyatlar",
      aiAssistant: "AI Maslahatchi",
      directory: "Ustalar katalogi",
      faq: "Qanday ishlaydi",
      login: "Kirish",
      joinAsMaster: "Usta sifatida qo'shilish"
    },
    footer: {
      copyright: "© 2026 UstaGo. Barcha huquqlar himoyalangan.",
      support: "Qo'llab-quvvatlash"
    },
    hero: {
      badge: "🛠 O'zbekistondagi #1 Usta topish platformasi",
      title: "Uyingizdagi har qanday muammoga professional usta — AI yordamida bir daqiqada.",
      subtitle: "Santexnik, elektrik, mebel yig'ish, remont va yuk tashish xizmatlari. Telefon qilib vaqt yo'qotmang — AI maslahatchi eng yaxshi ustani topib beradi.",
      findMaster: "Usta topish",
      howItWorks: "Qanday ishlaydi?"
    },
    categories: {
      all: "Barchasi",
      plumber: "Santexnik",
      electrician: "Elektrik",
      furniture: "Mebelchi",
      renovation: "Remont",
      mover: "Gruzchik"
    },
    comparison: {
      title: "UstaGo qanday farq qiladi?",
      beforeTitle: "OLDIN / QIYIN",
      before1: "Tanish-bilishdan soatlab usta qidirish va nomer topolmaslik.",
      before2: "Ustaning narxi va tajribasini oldindan bilmaslik, qimmat to'lash.",
      before3: "Telefon qilib tushuntirishdagi chalkashliklar va usta kechikishi.",
      before4: "Qilingan ishga kafolat yo'qligi va qayta topib bo'lmasligi.",
      afterTitle: "USTAGO BILAN / TEZ VA OSON",
      after1: "AI maslahatchi orqali muammoga qarab 10 soniyada usta tanlash.",
      after2: "Shaffof narxlar, haqiqiy mijozlar sharhlari va portfolio rasmlari.",
      after3: "Telegram orqali bitta klikda lokatsiya va rasmli buyurtma yuborish.",
      after4: "Har bir usta uchun shaxsiy mini-ilova va doimiy aloqa."
    },
    aiShowcase: {
      title: "Har bir xonadonga shaxsiy usta yordamchisi",
      feature1: "Sun'iy intellekt muammoingizni aniq tahlil qiladi",
      feature2: "Sizning hududingizdagi eng mos mutaxassislarni topadi",
      feature3: "Narxlar va reytinglarni solishtirib eng yaxshisini taklif etadi",
      userMessage: "Oshxonadagi kran tagidan suv oqyapti, zudlik bilan Chilonzorga usta kerak.",
      aiMessage: "Sizga yaqin joylashgan 3 ta eng reytingi baland santexnik topildi:\n1. Alisher Usta (⭐ 4.9, 15 daqiqada yetib boradi) — Narxi: 50,000 so'mdan.\nBog'lanish tugmasini bosing."
    },
    search: {
      placeholder: "O'zingizga kerakli mutaxassisni toping...",
      searchBtn: "Izlash",
      results: "ta natija topildi",
      noResults: "Natija topilmadi",
      startingPrice: "Boshlang'ich narx",
      viewProfile: "Profilni ko'rish",
      title: "Ustalar katalogi"
    },
    auth: {
      clientTab: "Mijoz sifatida",
      masterTab: "Usta sifatida",
      clientTitle: "Mijoz sifatida kirish",
      clientDesc: "Usta chaqirish uchun tizimga kiring",
      namePlaceholder: "Ism va Familiya",
      phonePlaceholder: "Telefon raqam (+998...)",
      loginBtn: "Kirish",
      masterTitle: "Usta sifatida kirish",
      masterDesc: "O'z kabinetingizga kiring",
      emailPlaceholder: "Email / Telefon",
      passwordPlaceholder: "Parol",
      masterLoginBtn: "Usta kabinetiga kirish",
      registerLink: "Yangi usta sifatida ro'yxatdan o'tish"
    },
    aiChat: {
      title: "AI Maslahatchi",
      status: "Onlayn",
      greeting: "Assalomu alaykum! Sizga qanday yordam bera olaman? Muammoingizni yozing.",
      placeholder: "Muammoingizni yozing (masalan: jo'mrak oqyapti)...",
      contactBtn: "Bog'lanish",
      profileBtn: "Profil"
    },
    dashboard: {
      title: "Boshqaruv Paneli",
      ordersTitle: "Mijozlar buyurtmalari",
      ordersTab: "Buyurtmalar",
      profileTab: "Profil",
      portfolioTab: "Portfolio",
      accept: "Qabul qilish",
      planActive: "Tarif: Faol",
      statusNew: "YANGI",
      statusInProgress: "JARAYONDA",
      statusCompleted: "YAKUNLANGAN",
      logout: "Chiqish",
      profilePreview: "Profil ko'rinishi (Mijozlar uchun)",
      editNoticeTitle: "Ma'lumotlarni o'zgartirish",
      editNoticeDesc: "Ma'lumotlarni o'zgartirish uchun adminga murojaat qiling",
      editNoticeBtn: "Adminga yozish",
      proTeaserTitle: "Profil dizaynini sozlash",
      proTeaserDesc: "PRO tarifida profil ranglari va foni dizaynini o'zgartirish imkoniyati mavjud. O'z profilingizni ajratib ko'rsating!",
      proTeaserBtn: "Tarifni yangilash →"
    },
    howPage: {
      title: "UstaGo qanday ishlaydi?",
      subtitle: "UstaGo — mijozlar va ustalar uchun eng qulay va zamonaviy platforma.",
      clientTitle: "Mijozlar uchun qo'llanma",
      clientStep1Title: "Tizimga kirish",
      clientStep1Desc: "Telefon raqamingiz yoki ijtimoiy tarmoqlar orqali bir zumda ro'yxatdan o'ting.",
      clientStep2Title: "Usta qidirish",
      clientStep2Desc: "Katalogdan kerakli mutaxassisni tanlang yoki muammoni yozib AI maslahatchidan yordam so'rang.",
      clientStep3Title: "Buyurtma berish",
      clientStep3Desc: "Usta bilan tanishib, to'g'ridan-to'g'ri bog'laning va xizmatga buyurtma bering.",
      masterTitle: "Ustalar uchun",
      masterDesc: "Usta sifatida platformaga qo'shilish va shaxsiy kabinet ochish faqat Admin orqali amalga oshiriladi.",
      masterPrice: "Akkaunt ochish narxi: 40 000 so'm (Chegirmada)",
      masterBtn: "Adminga yozish / Akkaunt ochish",
      aiTitle: "⚡ AI Powered",
      aiDesc: "Sun'iy intellekt (AI) tizimimiz muammoingizni chuqur tahlil qiladi va hududingizdagi eng mos mutaxassislarni 1 daqiqa ichida topib beradi. Vaqtingizni tejang va ishonchli usta toping!"
    },
    proPricing: {
      badge: "Ustalar Diqqatiga!",
      title: "Faqat ustalar uchun maxsus",
      subtitle: "UstaGo PRO orqali buyurtmalaringizni 5 barobarga oshiring",
      monthlyPlan: "1 Oylik PRO",
      monthlyDesc: "Barcha imkoniyatlarni sinab ko'rish uchun ajoyib boshlanish.",
      threeMonthPlan: "3 Oylik PRO",
      threeMonthBadge: "+1 OY BONUS",
      threeMonthDesc: "Eng mashhur tanlov. 3 oy to'lang, 4 oy ishlating.",
      sixMonthPlan: "6 Oylik PRO",
      sixMonthBadge: "+2 OY BONUS",
      sixMonthDesc: "Professional ustalar uchun uzoq muddatli va tejamkor variant.",
      lifetimePlan: "Umrbod PRO",
      lifetimeBadge: "CHEKSIZ",
      lifetimeDesc: "Bir marta to'lang, butun umr foydalaning. Hech qanday qo'shimcha to'lovlarsiz.",
      testPrice: "Test / Sinov narxi:",
      getPro: "Xarid qilish",
      buyPro: "PRO ga ulanish",
      feature1: "Katalogda doim 1-o'rinda turish",
      feature2: "Profil yonida moviy galochka",
      feature3: "AI qidiruvda birinchi bo'lib tavsiya etilish",
      feature4: "Kunlik cheksiz buyurtmalar olish",
      loginRequired: "Iltimos, avval usta profilingizga kiring"
    },
    proWidget: {
      activeDaysLeft: "⭐ PRO obuna faol: {days} kun qoldi",
      expiresAt: "(Tugash sanasi: {date})",
      warningLeft: "⚠️ PRO obunangiz tugashiga {days} kun qoldi! Imkoniyatlarni saqlab qolish uchun yangilang.",
      lastDay: "🚨 Bugun PRO obunangizning oxirgi kuni!",
      lifetime: "⭐ Umrbod PRO (Cheksiz muddat)",
      expired: "PRO muddati tugagan",
      renewBtn: "Yangilash",
      cancelBtn: "Obunani bekor qilish",
      cancelConfirm: "Rostdan ham PRO obunani bekor qilmoqchimisiz? To'langan mablag' qaytarib berilmaydi!",
      cancelSuccess: "PRO obunangiz bekor qilindi."
    },
    terms: {
      badge: "Xavfsizlik va Shaffoflik",
      title: "Foydalanish Qoidalari va To'lov Tartibi",
      desc: "UstaGo platformasida xizmatlardan foydalanish, to'lovlarni amalga oshirish va kafolat shartlari bilan tanishing.",
      card1Title: "💳 To'lov va PRO Xizmatini Faollashtirish Tartibi",
      card1Step1: "Bosqich 1: Kerakli PRO tarifni tanlaysiz va 'Xarid qilish' tugmasini bosasiz.",
      card1Step2: "Bosqich 2: Telegram bot orqali rasmiy hisob raqamimizga to'lov qilasiz.",
      card1Step3: "Bosqich 3: To'lov chekini botga yuborganingizdan so'ng, admin tasdiqlaydi va profilingiz bir zumda PRO maqomini oladi.",
      card2Title: "⚖️ Qoidalar va To'lov Qaytarilmasligi (Oferta)",
      card2Point1: "Qat'iy qoida: Tasdiqlangan va to'langan PRO paket puli hech qanday holatda QAYTARILMAYDI.",
      card2Point2: "Platforma qoidalarini (mijozlarni aldash, noto'g'ri xizmat ko'rsatish) qo'pol ravishda buzgan ustaning PRO statusi ogohlantirishsiz bekor qilinadi.",
      card2Point3: "O'z profilingiz ma'lumotlari (rasm, manzil) haqqoniyligiga to'liq javobgarsiz.",
      readMoreBtn: "Batafsil o'qish"
    }
  },
  RU: {
    navbar: {
      features: "Возможности",
      aiAssistant: "AI Помощник",
      directory: "Каталог мастеров",
      faq: "Как это работает",
      login: "Войти",
      joinAsMaster: "Стать мастером"
    },
    footer: {
      copyright: "© 2026 UstaGo. Все права защищены.",
      support: "Техническая поддержка"
    },
    hero: {
      badge: "🛠 Платформа #1 для поиска мастеров в Узбекистане",
      title: "Профессиональные мастера для любых задач дома — за минуту с помощью AI.",
      subtitle: "Услуги сантехника, электрика, сборщика мебели, ремонт и переезд. Не теряйте время на звонки — AI помощник найдет лучшего мастера.",
      findMaster: "Найти мастера",
      howItWorks: "Как это работает?"
    },
    categories: {
      all: "Все",
      plumber: "Сантехник",
      electrician: "Электрик",
      furniture: "Мебельщик",
      renovation: "Ремонт",
      mover: "Грузчик"
    },
    comparison: {
      title: "В чем отличие UstaGo?",
      beforeTitle: "РАНЬШЕ / СЛОЖНО",
      before1: "Часами искать мастеров через знакомых и не находить номер.",
      before2: "Не знать цену и опыт мастера заранее, переплачивать.",
      before3: "Путаница при объяснении проблемы по телефону и опоздания.",
      before4: "Нет гарантии на выполненную работу и невозможность найти мастера снова.",
      afterTitle: "С USTAGO / БЫСТРО И ПРОСТО",
      after1: "Выбор мастера за 10 секунд по вашей проблеме через AI помощника.",
      after2: "Прозрачные цены, реальные отзывы клиентов и портфолио работ.",
      after3: "Отправка заказа с фото и локацией в один клик через Telegram.",
      after4: "Личное мини-приложение для каждого мастера и постоянная связь."
    },
    aiShowcase: {
      title: "Личный помощник по дому для каждого",
      feature1: "Искусственный интеллект точно анализирует вашу проблему",
      feature2: "Находит наиболее подходящих специалистов в вашем районе",
      feature3: "Сравнивает цены и рейтинги, предлагая лучшее",
      userMessage: "Течет кран на кухне, срочно нужен мастер на Чиланзар.",
      aiMessage: "Найдены 3 сантехника с самым высоким рейтингом поблизости:\n1. Алишер Уста (⭐ 4.9, приедет через 15 минут) — Цена: от 50,000 сум.\nНажмите кнопку для связи."
    },
    search: {
      placeholder: "Найдите нужного специалиста...",
      searchBtn: "Поиск",
      results: "результатов найдено",
      noResults: "Результатов не найдено",
      startingPrice: "Начальная цена",
      viewProfile: "Профиль",
      title: "Каталог мастеров"
    },
    auth: {
      clientTab: "Как клиент",
      masterTab: "Как мастер",
      clientTitle: "Вход для клиента",
      clientDesc: "Войдите, чтобы вызвать мастера",
      namePlaceholder: "Имя и Фамилия",
      phonePlaceholder: "Номер телефона (+998...)",
      loginBtn: "Войти",
      masterTitle: "Вход для мастера",
      masterDesc: "Войдите в личный кабинет",
      emailPlaceholder: "Email / Телефон",
      passwordPlaceholder: "Пароль",
      masterLoginBtn: "Войти в кабинет",
      registerLink: "Зарегистрироваться как мастер"
    },
    aiChat: {
      title: "AI Помощник",
      status: "Онлайн",
      greeting: "Здравствуйте! Как я могу вам помочь? Опишите вашу проблему.",
      placeholder: "Опишите проблему (например: течет кран)...",
      contactBtn: "Связаться",
      profileBtn: "Профиль"
    },
    dashboard: {
      title: "Панель управления",
      ordersTitle: "Заказы клиентов",
      ordersTab: "Заказы",
      profileTab: "Профиль",
      portfolioTab: "Портфолио",
      accept: "Принять",
      planActive: "Тариф: Активен",
      statusNew: "НОВЫЙ",
      statusInProgress: "В ПРОЦЕССЕ",
      statusCompleted: "ЗАВЕРШЕН",
      logout: "Выйти",
      profilePreview: "Предпросмотр профиля (Для клиентов)",
      editNoticeTitle: "Редактирование данных",
      editNoticeDesc: "Для изменения информации обратитесь к администратору",
      editNoticeBtn: "Связаться с админом",
      proTeaserTitle: "Настройка дизайна профиля",
      proTeaserDesc: "На тарифе PRO доступно изменение цвета и фона профиля. Сделайте ваш профиль уникальным!",
      proTeaserBtn: "Обновить тариф →"
    },
    howPage: {
      title: "Как работает UstaGo?",
      subtitle: "UstaGo — самая удобная и современная платформа для клиентов и мастеров.",
      clientTitle: "Руководство для клиентов",
      clientStep1Title: "Авторизация",
      clientStep1Desc: "Зарегистрируйтесь мгновенно через номер телефона или соцсети.",
      clientStep2Title: "Поиск мастера",
      clientStep2Desc: "Выберите нужного специалиста из каталога или попросите AI помощника найти его для вас.",
      clientStep3Title: "Оформление заказа",
      clientStep3Desc: "Ознакомьтесь с профилем мастера, свяжитесь напрямую и оформите заказ.",
      masterTitle: "Для мастеров",
      masterDesc: "Регистрация в качестве мастера и открытие личного кабинета осуществляется только через Администратора.",
      masterPrice: "Стоимость открытия аккаунта: 40 000 сум (По скидке)",
      masterBtn: "Написать админу / Открыть аккаунт",
      aiTitle: "⚡ AI Powered",
      aiDesc: "Наша система искусственного интеллекта (AI) глубоко анализирует вашу проблему и находит наиболее подходящих специалистов в вашем районе за 1 минуту. Экономьте время и находите надежных мастеров!"
    },
    proPricing: {
      badge: "Внимание мастерам!",
      title: "Специально для мастеров",
      subtitle: "Увеличьте количество заказов в 5 раз с помощью UstaGo PRO",
      monthlyPlan: "1 Месяц PRO",
      monthlyDesc: "Отличное начало, чтобы опробовать все возможности.",
      threeMonthPlan: "3 Месяца PRO",
      threeMonthBadge: "+1 МЕСЯЦ БОНУС",
      threeMonthDesc: "Самый популярный выбор. Платите за 3, пользуйтесь 4.",
      sixMonthPlan: "6 Месяцев PRO",
      sixMonthBadge: "+2 МЕСЯЦА БОНУС",
      sixMonthDesc: "Долгосрочный и выгодный вариант для профессионалов.",
      lifetimePlan: "Пожизненный PRO",
      lifetimeBadge: "БЕЗЛИМИТ",
      lifetimeDesc: "Оплатите один раз, пользуйтесь всю жизнь. Никаких доплат.",
      testPrice: "Тестовая цена:",
      getPro: "Купить",
      buyPro: "Подключить PRO",
      feature1: "Всегда на 1-м месте в каталоге",
      feature2: "Синяя галочка рядом с профилем",
      feature3: "Приоритетные рекомендации от AI",
      feature4: "Неограниченное количество заказов в день",
      loginRequired: "Пожалуйста, сначала войдите в профиль мастера"
    },
    proWidget: {
      activeDaysLeft: "⭐ PRO активен: осталось {days} дней",
      expiresAt: "(До: {date})",
      warningLeft: "⚠️ До конца PRO осталось {days} дней! Обновите, чтобы сохранить преимущества.",
      lastDay: "🚨 Сегодня последний день вашего PRO!",
      lifetime: "⭐ Пожизненный PRO (Безлимит)",
      expired: "Срок действия PRO истек",
      renewBtn: "Обновить",
      cancelBtn: "Отменить подписку",
      cancelConfirm: "Вы уверены, что хотите отменить подписку PRO? Оплата не возвращается!",
      cancelSuccess: "Подписка PRO отменена."
    },
    terms: {
      badge: "Безопасность и прозрачность",
      title: "Условия оферты и правила оплаты",
      desc: "Ознакомьтесь с условиями использования, оплаты и гарантиями на платформе UstaGo.",
      card1Title: "💳 Порядок оплаты и активации",
      card1Step1: "Шаг 1: Выберите тариф PRO и нажмите «Купить».",
      card1Step2: "Шаг 2: Оплатите через Telegram бота на наш официальный счет.",
      card1Step3: "Шаг 3: После отправки чека админ подтвердит его, и ваш профиль мгновенно получит статус PRO.",
      card2Title: "⚖️ Правила и возврат средств (Оферта)",
      card2Point1: "Строгое правило: Оплата за подтвержденный пакет PRO НЕ ВОЗВРАЩАЕТСЯ.",
      card2Point2: "При грубом нарушении правил (обман, некачественный сервис) статус PRO аннулируется без предупреждения.",
      card2Point3: "Вы несете полную ответственность за достоверность данных в профиле.",
      readMoreBtn: "Читать подробнее"
    }
  },
  EN: {
    navbar: {
      features: "Features",
      aiAssistant: "AI Assistant",
      directory: "Masters Directory",
      faq: "How it works",
      login: "Sign in",
      joinAsMaster: "Join as Master"
    },
    footer: {
      copyright: "© 2026 UstaGo. All rights reserved.",
      support: "Customer Support"
    },
    hero: {
      badge: "🛠 #1 Craftsmen Platform in Uzbekistan",
      title: "Trusted craftsmen for every home repair task — in a minute with AI.",
      subtitle: "Plumbing, electrical, furniture assembly, renovation, and moving services. Don't waste time calling — let our AI assistant find the best master for you.",
      findMaster: "Find a Master",
      howItWorks: "How it works?"
    },
    categories: {
      all: "All",
      plumber: "Plumber",
      electrician: "Electrician",
      furniture: "Furniture Master",
      renovation: "Renovation",
      mover: "Mover"
    },
    comparison: {
      title: "How UstaGo is different?",
      beforeTitle: "BEFORE / HARD",
      before1: "Spending hours looking for craftsmen through acquaintances.",
      before2: "Not knowing the price and experience in advance, overpaying.",
      before3: "Confusion when explaining the problem over the phone and delays.",
      before4: "No guarantee for the work done and inability to find the master again.",
      afterTitle: "WITH USTAGO / FAST & EASY",
      after1: "Select a master in 10 seconds based on your problem via AI assistant.",
      after2: "Transparent prices, real customer reviews, and portfolio pictures.",
      after3: "Send an order with photos and location in one click via Telegram.",
      after4: "Personal mini-app for each master and permanent contact."
    },
    aiShowcase: {
      title: "Personal home assistant for everyone",
      feature1: "Artificial intelligence accurately analyzes your problem",
      feature2: "Finds the most suitable specialists in your area",
      feature3: "Compares prices and ratings to offer the best",
      userMessage: "The kitchen faucet is leaking, need a master in Chilanzar urgently.",
      aiMessage: "Found 3 top-rated plumbers nearby:\n1. Alisher Master (⭐ 4.9, arrives in 15 mins) — Price: from 50,000 UZS.\nClick to contact."
    },
    search: {
      placeholder: "Find the specialist you need...",
      searchBtn: "Search",
      results: "results found",
      noResults: "No results found",
      startingPrice: "Starting price",
      viewProfile: "View Profile",
      title: "Masters Directory"
    },
    auth: {
      clientTab: "As Client",
      masterTab: "As Master",
      clientTitle: "Client Login",
      clientDesc: "Log in to hire a master",
      namePlaceholder: "Full Name",
      phonePlaceholder: "Phone Number (+998...)",
      loginBtn: "Log In",
      masterTitle: "Master Login",
      masterDesc: "Log in to your dashboard",
      emailPlaceholder: "Email / Phone",
      passwordPlaceholder: "Password",
      masterLoginBtn: "Go to Dashboard",
      registerLink: "Register as a new Master"
    },
    aiChat: {
      title: "AI Assistant",
      status: "Online",
      greeting: "Hello! How can I help you today? Describe your problem.",
      placeholder: "Describe your problem (e.g. leaking faucet)...",
      contactBtn: "Contact",
      profileBtn: "Profile"
    },
    dashboard: {
      title: "Dashboard",
      ordersTitle: "Customer Orders",
      ordersTab: "Orders",
      profileTab: "Profile",
      portfolioTab: "Portfolio",
      accept: "Accept",
      planActive: "Plan: Active",
      statusNew: "NEW",
      statusInProgress: "IN PROGRESS",
      statusCompleted: "COMPLETED",
      logout: "Logout",
      profilePreview: "Profile Preview (For Clients)",
      editNoticeTitle: "Edit Information",
      editNoticeDesc: "To change profile details, contact the administrator",
      editNoticeBtn: "Contact Admin",
      proTeaserTitle: "Customize Profile Design",
      proTeaserDesc: "Change background themes and colors with PRO plan. Stand out from the crowd!",
      proTeaserBtn: "Upgrade Plan →"
    },
    howPage: {
      title: "How UstaGo works?",
      subtitle: "UstaGo — the most convenient and modern platform for clients and masters.",
      clientTitle: "Guide for Clients",
      clientStep1Title: "Sign In",
      clientStep1Desc: "Register instantly via your phone number or social networks.",
      clientStep2Title: "Find a Master",
      clientStep2Desc: "Select a specialist from the directory or ask the AI assistant to find one for you.",
      clientStep3Title: "Place an Order",
      clientStep3Desc: "Check out the master's profile, contact them directly and order the service.",
      masterTitle: "For Masters",
      masterDesc: "Joining as a master and opening an account is only possible through the Administrator.",
      masterPrice: "Account opening price: 40,000 UZS (Discounted)",
      masterBtn: "Contact Admin / Open Account",
      aiTitle: "⚡ AI Powered",
      aiDesc: "Our Artificial Intelligence (AI) system deeply analyzes your problem and finds the most suitable specialists in your area within 1 minute. Save time and find reliable craftsmen!"
    },
    proPricing: {
      badge: "Attention Masters!",
      title: "Special for Masters",
      subtitle: "Increase your orders by 5x with UstaGo PRO",
      monthlyPlan: "1 Month PRO",
      monthlyDesc: "A great start to try out all the features.",
      threeMonthPlan: "3 Months PRO",
      threeMonthBadge: "+1 MONTH BONUS",
      threeMonthDesc: "Most popular. Pay for 3 months, use for 4.",
      sixMonthPlan: "6 Months PRO",
      sixMonthBadge: "+2 MONTHS BONUS",
      sixMonthDesc: "Long-term and cost-effective for professionals.",
      lifetimePlan: "Lifetime PRO",
      lifetimeBadge: "UNLIMITED",
      lifetimeDesc: "Pay once, use forever. No extra charges.",
      testPrice: "Test Price:",
      getPro: "Purchase",
      buyPro: "Get PRO",
      feature1: "Always rank #1 in the directory",
      feature2: "Blue verified badge next to profile",
      feature3: "Priority recommendation by AI",
      feature4: "Unlimited daily orders",
      loginRequired: "Please log in to your master profile first"
    },
    proWidget: {
      activeDaysLeft: "⭐ PRO active: {days} days left",
      expiresAt: "(Expires: {date})",
      warningLeft: "⚠️ {days} days left until PRO expires! Renew to keep your benefits.",
      lastDay: "🚨 Today is the last day of your PRO subscription!",
      lifetime: "⭐ Lifetime PRO (Unlimited)",
      expired: "PRO subscription expired",
      renewBtn: "Renew",
      cancelBtn: "Cancel Subscription",
      cancelConfirm: "Are you sure you want to cancel your PRO subscription? Payments are non-refundable!",
      cancelSuccess: "PRO subscription cancelled."
    },
    terms: {
      badge: "Security & Transparency",
      title: "Terms & Conditions",
      desc: "Read the rules for using services, payments, and guarantees on UstaGo.",
      card1Title: "💳 Payment & Activation Process",
      card1Step1: "Step 1: Choose a PRO plan and click 'Purchase'.",
      card1Step2: "Step 2: Pay via Telegram bot to our official account.",
      card1Step3: "Step 3: After sending the receipt, admin confirms and your profile gets PRO status.",
      card2Title: "⚖️ Rules & Non-refundable Policy",
      card2Point1: "Strict rule: Payments for confirmed PRO packages are NON-REFUNDABLE under any circumstances.",
      card2Point2: "Severe rule violations (fraud, poor service) will result in PRO cancellation without warning.",
      card2Point3: "You are fully responsible for the accuracy of your profile data.",
      readMoreBtn: "Read More"
    }
  }
};
